...
def create_labelme_annotations(detected_objectss, filename, img_height, img_width):
    ...
    :param detected_objects: List of tuples with format (label, [(x1, y1), (x2, y2), ..., (xn, yn)])
    ...
    shapes = []
    for (label, points) in detected_objectss:
        shape = {
            "label": label,
...
def mouse_callback(event, x, y, flags, params):
    global detected_objects, global_img
    image = global_img
    pts = params[2]
    ...
    if len(pts) == 0:
        ...
        if event == cv.EVENT_LBUTTONDOWN:
            ...
            all_pts, image = generate_points_from_mask(image, masks)
            detected_objects.append(("default", all_pts))
            # 使用p1, p2与已经存在的box进行对比, 如果面积相近的话就视为同一个而不进行增加
            # image = apply_mask_on_image(image,  masks) 
            global_img = image
            cv.imshow(win_name, image)
            pts.clear()
            ...
            all_pts, image_copy = generate_points_from_mask(image_copy, masks)
...
def generate_box_from_mask(image, mask, random_color=True):
    if random_color:
...
def generate_points_from_mask(image, mask, random_color=True):
    if random_color:
        color = (np.random.randint(0, 255), np.random.randint(0, 255), np.random.randint(0, 255))
    else:
        color = (30, 144, 255)
    ...
    contours, _ = cv.findContours(matrix, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
    n = 5
    contours = [cnt[::n] for cnt in contours]
    ...
    for cnt in contours:
        ...
        # 绘制轮廓，获取轮廓的坐标点
        for point in cnt:
            points.append([int(x) for x in list(point[0])])

        # 更新最大外接矩形的坐标，画出外接矩形，这步可以根据需要去掉
        # x_min = min(x_min, x)
        # y_min = min(y_min, y)
        # x_max = max(x_max, x + w)
        # y_max = max(y_max, y + h)
    ...
    mask_fill = np.zeros(image.shape, dtype=np.uint8)
    alpha = 0.5
    cv.fillPoly(mask_fill, [tmp_points], color=color)
    image = cv.add(image, mask_fill)
...
def two_point_example(predictor, image_path, win_name="1"):
    ...
    while True:
        ...
        if key == 9:  # 当用户按下"Tab"键时退出
            ...
            if len(history) > 0 and len(detected_objects) > 0:
